...
def add_keydate(request, case_id=0, keydate_id=None):
    ...
    if request.method == "POST":
        ...
        payment_form = KeydateForm(initial={'add_user': user.id, 'client':client_id, 'case':case_id}, instance=keydate)
...
def add_payment(request, case_id=0, payment_id=None):
    ...
    if request.method == "POST":
        ...
        if payment:
            ...
            case = get_object_or_404(Case, pk=case_id)
            client_id = case.client.id
        payment_form = PaymentForm(initial={'add_user': user.id, 'client':client_id, 'case':case_id}, instance=payment)
